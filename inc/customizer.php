...
function poppy_whatsapp_floating_button(): void {
	...
	$position_class = 'md:right-6';
	$flex_row_class = 'flex-row';
	if ( 'bottom_left' === $position ) {
		$position_class = 'md:left-6';
		$flex_row_class = 'flex-row-reverse';
	}
	?>
	<!-- Floating WhatsApp Button -->
	<div class="fixed bottom-0 left-0 right-0 md:left-auto md:bottom-6 <?php echo esc_attr( $position_class ); ?> z-[9999] md:flex md:items-center md:gap-3 group pointer-events-auto select-none">
		<a href="<?php echo esc_url( $url ); ?>" target="_blank" rel="noopener noreferrer" class="whatsapp-floating-action w-full md:w-auto flex items-center justify-between md:justify-start <?php echo esc_attr( $flex_row_class ); ?> gap-4 text-white transition duration-300 focus:outline-none">
			
			<?php if ( ! empty( $tooltip ) ) : ?>
				<!-- Tooltip Text -->
				<span class="hidden md:inline-flex bg-white text-poppy-ink shadow-lg rounded-xl px-4 py-2 text-xs font-bold border border-slate-100/80 opacity-0 transform translate-y-2 scale-95 transition-all duration-300 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100">
					<?php echo esc_html( $tooltip ); ?>
				</span>
			<?php endif; ?>

			<!-- Floating WhatsApp Icon Wrapper -->
			<div class="order-2 md:order-none w-12 h-12 md:w-14 md:h-14 bg-white md:bg-[#25D366] md:hover:bg-[#20BA5A] text-poppy-accent md:text-white rounded-full flex-shrink-0 flex items-center justify-center shadow-md md:shadow-xl md:hover:shadow-2xl md:hover:scale-110 transition duration-300 ease-in-out cursor-pointer relative">
				<!-- WhatsApp SVG Icon -->
				...
				<!-- Ping attention dot -->
				<span class="hidden md:flex absolute top-0 right-0 h-3.5 w-3.5 translate-x-1/4 -translate-y-1/4 z-10">
					<span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
					<span class="relative inline-flex rounded-full h-3.5 w-3.5 bg-red-500"></span>
				</span>
			</div>

			<span class="order-1 md:hidden text-sm sm:text-base font-extrabold tracking-wide">Tanya Airlangga via WhatsApp</span>
		</a>
	</div>
	<?php
}
