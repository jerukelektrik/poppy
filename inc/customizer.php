...
function poppy_whatsapp_floating_button(): void {
	...
	$desktop_position_class = 'md:right-6 md:left-auto';
	$flex_row_class = 'flex-row';
	if ( 'bottom_left' === $position ) {
		$position_class = 'left-6';
		$desktop_position_class = 'md:left-6 md:right-auto';
		$flex_row_class = 'flex-row-reverse';
	}
	?>
	<!-- Floating WhatsApp Button — Mobile banner, desktop floating icon -->
	<div class="fixed bottom-0 left-0 right-0 md:bottom-6 <?php echo esc_attr( $desktop_position_class ); ?> z-[9999] flex items-center gap-3 group pointer-events-auto select-none">
		...
		<a href="<?php echo esc_url( $url ); ?>" target="_blank" rel="noopener noreferrer"
			class="whatsapp-floating-action w-full md:w-14 md:h-14 md:rounded-full flex items-center justify-between md:justify-center shadow-lg md:hover:shadow-xl md:hover:scale-110 transition duration-300 focus:outline-none relative <?php echo 'bottom_left' === $position ? 'md:order-1' : ''; ?>"
		>
			<span class="md:hidden text-white text-lg sm:text-xl font-black leading-tight">
				Tanya Airlangga via WhatsApp
			</span>

			<span class="whatsapp-floating-icon-wrap relative flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-full bg-white shadow-md md:h-auto md:w-auto md:bg-transparent md:shadow-none">
				<!-- WhatsApp SVG Icon -->
				<svg class="whatsapp-floating-icon w-7 h-7 md:w-7 md:h-7" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
